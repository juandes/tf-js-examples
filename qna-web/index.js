...
const question = 'When was Tesla born?';
const passage = `
...
function createTestButtons(innerText, selector, id, listener, disabled = false) {
  const btn = document.createElement('BUTTON');
...
async function init() {
  ...
  createTestButtons('Tesla', '#test-buttons', 'test-case-tesla',
    async () => {
      document.getElementById('input-text').value = 'heeyyy';
    }, false);
